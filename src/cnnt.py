...
if args.pretrained:
    project_name = "resnet_101_pretrained"
else:
    project_name = "resnet_101"
...
class Pad(nn.Module):
    def __init__(self, kernel_size):
        ...
        self.p2d = nn.ZeroPad2d(reduce(__add__,
...
#             Pad(kernel_size=(2,2)),
            ...
            ##CNN Layer 3
            nn.Dropout(.2),
            nn.Conv2d(in_channels=32,out_channels=48, kernel_size=3, stride=1, padding="same"),
            nn.BatchNorm2d(48),            
            nn.Mish(),    
#             Pad(kernel_size=(2,2)),
            nn.MaxPool2d(kernel_size=(2,2), stride=2), 
            ##CNN Layer 4
            nn.Dropout(.5),
            nn.Conv2d(in_channels=48,out_channels=64, kernel_size=3, stride=1, padding="same"),
            nn.BatchNorm2d(64),            
            nn.Mish(),
#             Pad(kernel_size=(2,2)),
#             nn.MaxPool2d(kernel_size=(1,1), stride=1),
            ##CNN Layer 5
            nn.Dropout(.5),
            nn.Conv2d(in_channels=64,out_channels=80, kernel_size=3, stride=1, padding="same"),            
            nn.BatchNorm2d(80),                        
            nn.Mish(),
#             Pad(kernel_size=(2,2)),
#             nn.MaxPool2d(kernel_size=(1,1), stride=1),
...
valid_sample = DistributedSampler(DataGenerator(source_path,'valid',transform_valid, tokenizer))

train_loader = torch.utils.data.DataLoader(DataGenerator(source_path,'train',transform_train, tokenizer), batch_size=batch_size, sampler=train_sample, num_workers=0)
...
val_loader = torch.utils.data.DataLoader(DataGenerator(source_path,'valid',transform_valid, tokenizer), batch_size=batch_size,sampler=valid_sample, num_workers=0)
...
for epoch in range(epochs):
    ...
    valid_loss = torch.Tensor([valid_loss]).to(local_rank)
    cer = torch.Tensor([cer]).to(local_rank)    
    dist.all_reduce(valid_loss,op=dist.ReduceOp.SUM)
    dist.all_reduce(cer, op=dist.ReduceOp.SUM)
    valid_loss = (valid_loss/4).item()    
    cer = (cer/4).item()
    if local_rank==0:
...
#             c=0
    ...
    if epoch%10==0 and epoch!=0 and save==0 and local_rank==0:
